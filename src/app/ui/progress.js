...
module.exports = class Progress {
  ...
  static text (bar) {
    return bar.attrs.text || (bar.id.toString().replace("pbar", "").toLowerCase() + " " + percent)
  }

  view ({attrs}) {
    const bar     = attrs.bar || {}
    const percent = "percent" in attrs ? attrs.percent : Progress.parse_percentage(bar)
    const text    = "text" in attrs ? attrs.text : Progress.text(bar)
        
    return m(`li#${bar.id}`,
      [ m(`.bar.${Progress.classify(percent)}`, {style: {width: percent.toString() + "%" }})
      , m(".value", text)
      ])
  }
}