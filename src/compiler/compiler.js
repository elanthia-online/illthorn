...
const LinkRegex = /(\b(?:https?):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gi

const UNTRUSTED_TAG_IDS = { speech: 1, thoughts: 1 }
...
module.exports = class Compiler {
  ...
  static cannot_compile(tag) {
    return (
      Compiler.is_doubled(tag) || !Compiler.can_compile(tag)
    )
  }

  static is_doubled(tag) {
    return (
      ~tag.name.toLowerCase().indexOf("stream") &&
      tag.id in DuplicateStream
    )
  }

  static compile(parent, cb) {
    Compiler.compile_child_substr(parent, (children) => {
  ...
  static compile_child_substr(parent, cb) {
    requestAnimationFrame(function () {
      ...
      if (Settings.get("compiler.run", true) === false)
        return cb(parent.text)
      if (
        parent.text.length >
        Settings.get("compiler.max_length", 800)
      )
        return cb(parent.text)
      ...
      return cb(
        (parent.children || []).reduce(
          (compiler, tag) => {
            const before = compiler.text.substr(
              0,
              tag.start + compiler.offset
            )
            const after = compiler.text.substr(
              tag.end + compiler.offset,
              compiler.text.length
            )
            const {
              hilited,
              offset,
            } = Compiler.compile_a_tag(
              tag.id || tag.name,
              tag.attrs,
              tag.text
            )
            ...
            return {
              text,
              offset: offset + compiler.offset,
            }
          },
          { offset: 0, text: parent.text }
        ).text
      )
    })
  }
  ...
  static add_hilites_v2(
    parent,
    hilites = Hilites.get(),
    depth = 0
  ) {
    if (depth >= Settings.get("compiler.max_depth", 4))
    ...
    while (i < length) {
      ...
      if (node.nodeName == "#text")
        Compiler.hilite_safe_text_node(
          parent,
          node,
          hilites,
          depth + 1
      ...
      if (node.nodeName == "A")
        Compiler.hilite_exist_tag(node, hilites, depth + 1)
      if (node.nodeName == "INS") continue // already processed
    }
  }

  static hilite_safe_text_node(
    parent,
    text_node,
    hilites,
    depth
    ...
    let i = 0
    const text = text_node.substringData(
      0,
      text_node.length
    )
    ...
    while (i < hilites.length) {
      ...
      if (text.match(pattern)) {
        const update = text.replace(pattern, function (
          match
        ) {
          return `<ins class="${group}">${match}</ins>`
        })
        const hilited = document.createElement("span")
        hilited.innerHTML = update
        parent.replaceChild(hilited, text_node)
        return Compiler.add_hilites_v2(
          hilited,
          hilites,
          depth + 1
        )
  ...
  static hilite_exist_tag(a, hilites) {
    a.innerHTML = Compiler.hilite_unsafe_text(
      a.innerText,
      hilites
    )
  }
    ...
    return str.split(LinkRegex).map((part) => {
      if (
        ...
        return m(
          ...
          {
            href: "#",
            onclick: open_external_link.bind(0, part),
          },
          "link:" + part
        )
      }
  ...
  static compile_a_tag(kind, attrs = {}, text) {
    const [start, end] = [
      `<a${Compiler.compile_attrs(kind, attrs)}>`,
      `</a>`,
    ]
    ...
    return (
      ...
      Object.keys(attrs).reduce(
        (acc, attr) =>
          acc +
          " " +
          `data-${attr}=${JSON.stringify(attrs[attr])}`,
        ""
      )
    )
  }
