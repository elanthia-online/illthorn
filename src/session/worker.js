...
class Bridge {
  ...
  static connect (port) {
    Bridge.parser.on("tag", tag => self.postMessage({topic: "TAG", gram: tag}))
...
self.onmessage = ({data}) => {
  switch (data.topic) {
    case "CONNECT" : return Bridge.connect(data.port)
    case "COMMAND" : return Bridge.sock && Bridge.sock.write(data.command + "\r\n")
  }
}