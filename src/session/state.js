...
module.exports = class SessionState {
  static TAGS = ["prompt", "right", "left", "spell", "roundtime", "compass"];
  ...
  static consume(state, tag) {
    ...
    if (tag.children && tag.children.length)
      tag.children.forEach((child) => SessionState.consume(state, child));
  }
  ...
  wire_up() {
    ...
    parser.on("NOTIFICATION", (tag) => {
      ...
      if (tag.text)
        new Notification(
          this._session.name + " " + Lens.get(tag, "attrs.title"),
