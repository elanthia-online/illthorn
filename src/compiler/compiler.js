...
const DuplicateStream = { speech: 1, room: 1, inv: 1, bounty: 1 };
...
module.exports = class Compiler {
  ...
  static is_doubled(tag) {
    return (
      ~tag.name.toLowerCase().indexOf("stream") && tag.id in DuplicateStream
    );
  }
  ...
  static compile_child_substr(parent, cb) {
    requestAnimationFrame(function () {
      ...
      if (parent.text.length > Settings.get("compiler.max_length", 800))
      ...
      return cb(
        (parent.children || []).reduce(
          (compiler, tag) => {
            ...
            const { hilited, offset } = Compiler.compile_a_tag(
              tag.id || tag.name,
              tag.attrs,
              tag.text
            );
  ...
  static add_hilites_v2(parent, hilites = Hilites.get(), depth = 0) {
    ...
    while (i < length) {
      ...
      if (node.nodeName == "#text")
        Compiler.hilite_safe_text_node(parent, node, hilites, depth + 1);
      if (node.nodeName == "A")
  ...
  static hilite_safe_text_node(parent, text_node, hilites, depth) {
    ...
    while (i < hilites.length) {
      ...
      if (text.match(pattern)) {
        const update = text.replace(pattern, function (match) {
  ...
  static compile_attrs(kind, attrs) {
    ...
    return (
      ...
      Object.keys(attrs).reduce(
        (acc, attr) =>
          acc + " " + `data-${attr}=${JSON.stringify(attrs[attr])}`,
        ""
      )
    );
  }
