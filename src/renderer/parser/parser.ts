...
export class Parser {
  ...
  parse (text : string) {
    ...
    while (cursor.idx < text.length) {
      ...
      char == "<"
        ? this.handleTagOpen(cursor, text)
  ...
  handleTagOpen (cursor : ParserCursor, text : string) {
    const tagInfo = this.consumeToChar(cursor, text,
      {delimiter: ">", inclusive: true})
    //console.log("tag/open -> ", tagInfo, cursor)
  ...
  closeTag (tagInfo : string) : void {
    const cursor = {idx: 0}
    const tagName = normalizeTagName(this.consumeToChar(cursor, tagInfo,
      {delimiter: " ", inclusive: false}))
    ...
    const closedTag = makeTag(tagName)
  ...
  openTag (tagInfo : string) {
    ...
    const tag = makeTag(tagName as TagName)
    if (tagName.length < tagInfo.length) {
